...
// --- NEW MODAL LOGIC ---
// --- RADICAL FIX: INLINE MODAL ---
window.openBonusHistoryModal = async function () {
    // Remove if exists to be safe
    const ex = document.getElementById('bonus-history-modal-fixed');
    if (ex) ex.remove();

    // Create Modal Element
    const modal = document.createElement('div');
    modal.id = 'bonus-history-modal-fixed';
    ...
    modal.innerHTML = `
        ...
        ">
            <h3 style="color: #ffd700; margin-top: 0; text-align: center;">История баллов</h3>
            <button id="close-history-btn" style="
                position: absolute; top: 10px; right: 15px; background: none; border: none;
                color: #fff; font-size: 24px; cursor: pointer;
            ">&times;</button>
            <div id="bonus-history-container-fixed" style="margin-top: 15px;">
    ...
    // Fetch Data
    const container = document.getElementById('bonus-history-container-fixed');
            ...
            history.forEach(tx => {
                ...
                container.innerHTML += `
                    <div style="display: flex; justify-content: space-between; padding: 10px 0; border-bottom: 1px solid #333;">
                        <div>
                            <div style="font-size: 0.9em;">${tx.description || 'Транзакция'}</div>
                            <div style="font-size: 0.8em; color: #888;">${date}</div>
                        </div>
                        <div style="color: ${color}; font-weight: bold;">${sign}${Math.abs(tx.amount)}</div>
                    </div>
                `;
            });
        }
