-- ИСПРАВЛЕНИЕ РЕФЕРАЛЬНОЙ ЛОГИКИ (Финал)
-- Правило: 100 бонусов получают ОБА (и пригласивший, и приглашенный), но только если приглашение БЫЛО.
-- Если пришел сам -> только 2% кэшбэк.
...
BEGIN
    ...
    -- А) ОБЩИЙ КЭШБЭК (2%) - Начисляется всем и всегда
    ...
    -- Б) РЕФЕРАЛЬНАЯ ПРОГРАММА (Работает только если есть referrer_id!)
    IF client_rec.referrer_id IS NOT NULL THEN
        
        -- Если это ПЕРВЫЙ заказ, даем бонусы за дружбу
        IF new_total_orders = 1 THEN
            
            -- 1. Начисляем НОВИЧКУ (Коле) +100 бонусов
            -- Проверяем, не получал ли он их случайно (хотя это 1-й заказ, но мало ли)
            IF NOT EXISTS (SELECT 1 FROM bonus_transactions WHERE user_id = ord.user_id AND description = 'Welcome Bonus (Referral)') THEN
                UPDATE clients 
                ...
                INSERT INTO bonus_transactions (user_id, amount, description)
                VALUES (ord.user_id, 100, 'Welcome Bonus (Referral)');
            END IF;

            -- 2. Начисляем РЕФЕРЕРУ (Васе) +100 бонусов
        ...
        -- 3. Начисляем РЕФЕРЕРУ (Васе) % с заказа (1%)
